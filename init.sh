...
function init_hal_gralloc()
{
	case "$(cat /proc/fb | head -1)" in
		0*inteldrmfb|0*radeondrmfb|0*nouveaufb|0*amdgpudrmfb)
...
function do_bootcomplete()
{
	...
	lsmod | grep -e brcmfmac -e rtl8723be && setprop wlan.no-unload-driver 1
...
#	[ -d /proc/asound/card0 ] || modprobe snd-dummy
	...
	/system/bin/sh /system/linux/sea/keeper/seafile_keeper >> /cache/keeper.log 2>&1 &
}
